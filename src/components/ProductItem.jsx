...
const ProductItem = ({ filterData }) => {
  ...
  return (
    <div className='w-full md:w-[80%] grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-7'>
      {
        filteredAllData?.map((product, idx) => (
          <div key={idx} onClick={() => goDetails(idx, product)} className='bg-white h-fit dark:bg-gray-900 shadow-lg shadow-black/50 rounded-xl p-3 flex flex-col gap-2 cursor-pointer hover:scale-105 transform transition-all duration-200'>
